...
test("WmlToHtmlConverter: honors w:start for ordered lists", async () => {
...
`;
  ...
  assert.match(res.html, /<ol[^>]*start="5"/);
  assert.match(res.html, />#5</);
});

test("WmlToHtmlConverter: honors w:startOverride for ordered lists", async () => {
  const numberingXml = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<w:numbering xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
  <w:abstractNum w:abstractNumId="1">
    <w:lvl w:ilvl="0">
...
`;
  ...
  assert.match(res.html, /<ol[^>]*start="7"/);
  assert.match(res.html, />#7</);
});
